...
## **_RICORSIONE_**
_esempio_: recurse_dir.sh
script di esempio che emula il comportalmento di ls -R, esplorando ricorsivamente ogni sottodirettorio 

```bash
#!/bin/bash

# CASO BASE: Se l'argomento non è un direttorio, è un file
if ! test -d "$1" ; then
    echo $(pwd)/$1
# CASO RICORSIVO: Se è un direttorio, entra ed esplora
else
    cd "$1"
    for f in * ; do
        # Invocazione ricorsiva usando $0
        "$0" "$f"
    done
    cd .. # Torna su per permettere la corretta risalita dell'albero
fi
```
...
#Determinare se $= è assoluto o reltivo prima di chiamare la ricorsione
if [[ "$0" = /* ]] ; then
    # Caso 1: $0 è un percorso ASSOLUTO (inizia con /)
    dir_name=$(dirname "$0")
    recursive_cmd="$dir_name/do_recurse_dir.sh"

elif [[ "$0" = */* ]] ; then
    # Caso 2: $0 è un percorso RELATIVO (contiene uno slash ma non all'inizio)
    dir_name=$(dirname "$0")
    recursive_cmd="$(pwd)/$dir_name/do_recurse_dir.sh"

else
    # Caso 3: Il comando è nel $PATH (non contiene slash)
    recursive_cmd="do_recurse_dir.sh"
fi

# Invocazione del comando ricorsivo vero e proprio
"$recursive_cmd" "$1"
```
